...
export function mountWorkspace(workspaceHtml) {
    ...
    // Click anywhere on a task row (in either the main list or the subtasks
    // sub-columns) to select it. Skip only the mutating affordances since
    // those have semantics of their own. Description clicks DO select; focus
    // follows naturally because the refresh below only swaps the right
    // column's contents and a CSS class.
    $('#objective-workspace').on('mousedown.workspace-select', '.objective-task-item', (e) => {
        const $t = $(e.target);
        if ($t.is('input[type="checkbox"]')) return;
        if ($t.closest('[id^=objective-task-delete-],[id^=objective-task-add-],[id^=objective-task-duration-]').length) return;
