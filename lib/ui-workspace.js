...
export function mountWorkspace(workspaceHtml) {
    ...
    // Click anywhere on a task row's non-interactive area to select it.
    // Each task row gets re-rendered on every updateUiTaskList(), so we
    // bind via delegation on the persistent #objective-tasks container.
    $('#objective-tasks').on('mousedown.workspace-select', '.objective-task-item', (e) => {
        const $t = $(e.target);
        // Skip if the click landed on something that has its own behavior.
        if ($t.is('input, textarea, select, button, [contenteditable="true"]')) return;
        if ($t.closest('[id^=objective-task-drag-],[id^=objective-task-delete-],[id^=objective-task-add-],[id^=objective-task-add-branch-],[id^=objective-task-duration-]').length) return;
