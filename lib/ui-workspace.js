...
import { attachSortable, onTopLevelReorder } from './ui-tasklist.js';
...
export function mountWorkspace(workspaceHtml) {
    ...
    $('#objective-workspace-overflow-menu').on('click', '.menu_button', () => {
        // The clicked item's own handler runs separately; we just dismiss.
        closeOverflowMenu();
    });
...
/**
 * Render the Completed column with the completed top-level tasks from the
 * tree (not `state.recentlyCompletedTasks`, which is the prompt-injection
 * cache and stays separate). Each row uses the full task template so the
 * checkbox doubles as an "uncomplete" affordance — unchecking flips the
 * task back to incomplete, and the next refresh moves it to Upcoming.
 */
function renderRecentlyCompleted() {
    ...
    // Drag-reorder shares the top-level merge logic — Upcoming and Completed
    // both slice the same children array, so a drop in either rebuilds it.
