...
export function mountWorkspace(workspaceHtml) {
    ...
    $('#objective-task-detail-description').on('focusout', () => {
        const task = getDetailTask();
        if (!task) return;
        const next = $('#objective-task-detail-description').val();
        if (task.description !== next) {
            task.description = next;
            saveState();
            // Reflect into the underlying card; updateUiTaskList re-renders it.
            updateUiTaskList();
        }
    });
...
function renderTaskDetail() {
    ...
    $('.objective-task-detail-title').text('Task Detail');
    $('#objective-task-detail-description').val(task.description ?? '');

    // Breadcrumb: each ancestor is a clickable link, current task plain.
    ...
    chain.forEach((ancestor) => {
        ...
        $crumb.append(link).append(' › ');
    });
    $crumb.append(
        $('<span>').addClass('objective-task-detail-breadcrumb-current')
            .text(task.description?.trim() || '(unnamed)')
    );
