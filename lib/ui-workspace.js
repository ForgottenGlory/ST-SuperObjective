...
export function mountWorkspace(workspaceHtml) {
    ...
    $('#objective-task-detail-title').on('change', () => {
        const task = getDetailTask();
        if (!task) return;
        const next = $('#objective-task-detail-title').val().trim();
