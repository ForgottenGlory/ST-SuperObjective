...
export function mountWorkspace(workspaceHtml) {
    ...
    // Click anywhere on a task row to select it. Skip only the mutating
    // affordances (checkbox toggle, delete, add, branch, duration) since
    // those have semantics of their own that don't imply "view this task."
    // Description clicks DO select, then focus follows for editing — the
    // refresh below only mutates the right column + a CSS class, so the
    // description's natural focus on click is unaffected.
    $('#objective-tasks').on('mousedown.workspace-select', '.objective-task-item', (e) => {
        const $t = $(e.target);
        if ($t.is('input[type="checkbox"]')) return;
        if ($t.closest('[id^=objective-task-delete-],[id^=objective-task-add-],[id^=objective-task-add-branch-],[id^=objective-task-duration-]').length) return;
...
function renderSubtasksPanel() {
    ...
    const completed = selected.children.filter(c => c.completed);
    const upcoming  = selected.children.filter(c => !c.completed);

    const renderCard = (t) => `
        <div class="objective-workspace-card objective-workspace-subtask-card${t.completed ? ' is-completed' : ''}"
             data-task-id="${t.id}" title="Click to select">
            <div class="objective-workspace-card-text">${escapeHtml(t.description || '')}</div>
            ${t.children?.length ? `<div class="objective-workspace-card-meta">${t.children.length} sub${t.children.length === 1 ? 'task' : 'tasks'}</div>` : ''}
        </div>
    `;

    completedList.html(completed.map(renderCard).join('') || '<div class="objective-workspace-empty">—</div>');
    upcomingList.html(upcoming.map(renderCard).join('')   || '<div class="objective-workspace-empty">—</div>');

    $('.objective-workspace-subtask-card').off('click.subtask').on('click.subtask', function () {
        const id = Number($(this).data('task-id'));
        if (!Number.isNaN(id)) setSelectedTask(id);
    });
}
