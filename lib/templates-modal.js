...
import { ObjectiveTask } from './task.js';
...
async function loadTaskTemplate() {
    ...
    for (const data of templateTasks) {
        ...
        if (data.children && data.children.length > 0) {
            loadChildTasksRecursive(task, data.children);
        }
...
function loadChildTasksRecursive(parentTask, childrenData) {
    for (const data of childrenData) {
        const child = new ObjectiveTask({ description: data.description, parentId: parentTask.id });
        if (data.children && data.children.length > 0) {
            loadChildTasksRecursive(child, data.children);
        }
