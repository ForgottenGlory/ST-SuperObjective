...
export class ObjectiveTask {
    ...
    addUiElement(container = '#objective-tasks') {
        ...
        const completedCount = this.children?.filter(c => c.completed).length ?? 0;
        const subtaskHint = childCount > 0
            ? `<small class="objective-task-card-hint">Subtasks: ${completedCount}/${childCount}</small>`
            : '';
        ...
        <div id="objective-task-item-${this.id}" class="objective-task-item">
            ...
            <div class="objective-task-card-body">
