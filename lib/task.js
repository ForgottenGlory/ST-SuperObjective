...
export class ObjectiveTask {
    ...
    /**
     * Render this task's row into the given container and wire up its handlers.
     * Defaults to the main task list; the workspace's subtasks panel passes
     * its own container so subtasks get the same affordances as top-level tasks.
     *
    ...
    addUiElement(container = '#objective-tasks') {
        ...
        <div id="objective-task-item-${this.id}" class="objective-task-item">
            <div id="objective-task-label-${this.id}" class="flex1 checkbox_label alignItemsCenter">
                ...
                <div id="objective-task-add-${this.id}" class="objective-task-button fa-solid fa-plus fa-fw fa-lg" title="Add Sibling"></div>
    ...
    onCompleteClick() {
        ...
        // updateUiTaskList re-applies the highlight via setCurrentTask,
        // preserving the existing current task when valid — so unchecking
        // a task no longer yanks focus to it.
        this.checkParentComplete();
        updateUiTaskList();
    }
